    ...
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-10">
            <div class="flex justify-center items-center gap-4 mb-4 flex-wrap">
                <img src="{{ asset('rpl.png') }}" alt="Logo TEFA" class="w-16 h-16 object-contain drop-shadow-xl shrink-0">
                <div class="w-px h-8 bg-gray-300 dark:bg-gray-700 hidden sm:block"></div>
                <img src="{{ asset('labantik.png') }}" alt="Logo Sekolah"
        ...
        <div
            ...
            <form method="POST" action="{{ route('login.store') }}" class="space-y-6" x-data="{ loading: false }"
                x-on:submit="setTimeout(() => loading = true, 50)">
                @csrf

                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2 ml-1">Email
                        Admin</label>
                    <div class="relative">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus
                            ...
                            class="w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary-blue dark:text-white transition-all">
                    </div>
                    @error('email')
                        <p class="text-xs text-primary-red mt-2 font-bold">{{ $message }}</p>
                    @enderror
                </div>

                <div>
                    <label
                        class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2 ml-1">Password</label>
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center justify-between pt-2">
                    <label class="flex items-center group cursor-pointer">
                        <input type="checkbox" name="remember"
                            class="rounded-lg text-primary-blue focus:ring-primary-blue border-gray-200 dark:bg-gray-900">
                        <span class="ml-2 text-xs font-bold text-gray-400 group-hover:text-gray-600 transition">Ingat
                            Saya</span>
                    </label>
                </div>

                <button type="submit"
                    class="w-full py-5 bg-primary-blue hover:bg-blue-900 text-primary-yellow rounded-2xl font-black text-lg shadow-xl shadow-blue-900/20 active:scale-95 transition-all uppercase italic tracking-wider disabled:opacity-50 disabled:cursor-not-allowed">
