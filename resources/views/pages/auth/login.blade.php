...
<body x-data="{
    ...
    <div class="h-full min-h-full grid grid-cols-1 lg:grid-cols-2 overflow-hidden">
        ...
        <div class="hidden lg:flex relative flex-col items-center justify-center p-12 lg:p-16 text-center text-white overflow-hidden z-10"
            style="background: linear-gradient(135deg, #1e3a8a 0%, #2563eb 50%, #1d4ed8 100%);">
            <!-- Ambient Glow Effects -->
            <div class="absolute -top-32 -left-32 w-96 h-96 rounded-full blur-3xl opacity-30 pointer-events-none"
                style="background-color: #60a5fa;"></div>
            <div class="absolute -bottom-32 -right-32 w-96 h-96 rounded-full blur-3xl opacity-30 pointer-events-none"
            ...
            <div class="relative z-10 max-w-md mx-auto flex flex-col items-center justify-center">
                ...
                <!-- Title -->
                <h1
                    class="text-4xl lg:text-5xl font-black tracking-tight mb-4 text-white drop-shadow-md uppercase italic">
                    SUPERAPPS TEFA
                </h1>

                <!-- Brief Description -->
                <p class="text-sm lg:text-base font-medium leading-relaxed max-w-sm"
        ...
        <div
            class="relative h-full flex flex-col justify-between items-center p-6 sm:p-10 lg:p-16 bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden">

            <!-- Row 1: Top Bar / Dark Mode Toggle -->
            <div class="w-full flex justify-end items-center z-20 shrink-0">
                <button @click="toggleTheme()" type="button" title="Beralih Mode Gelap/Terang"
                    ...
                    <svg x-show="!darkMode" class="w-4 h-4 text-slate-600" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="w-full max-w-sm sm:max-w-md my-auto flex flex-col justify-center py-4">
                ...
                <div class="mb-6 sm:mb-8 text-left">
                    <h2 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Selamat
                        Datang!</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-xs sm:text-sm mt-1.5 font-medium">Silakan masuk
                        untuk mulai mencatat transaksi.</p>
                </div>

                <!-- Form -->
                <form method="POST" action="{{ route('login.store') }}" class="space-y-4 sm:space-y-6"
                    x-data="{ loading: false }" x-on:submit="setTimeout(() => loading = true, 50)">
                    @csrf

                    <div>
                        <label
                            class="block text-xs font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-2 ml-1">
                            Email Admin
                        </label>
                        <div class="relative">
                            <span
                                class="absolute inset-y-0 left-0 pl-4 flex items-center text-slate-400 dark:text-slate-500 pointer-events-none">
                    ...
                    <div class="flex items-center justify-between pt-1">
                        <label class="flex items-center group cursor-pointer">
                            ...
                            <span
                                class="ml-2.5 text-xs font-bold text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-300 transition-colors">
                                Ingat Saya
                            </span>
                        </label>
                    </div>
            ...
            <footer class="w-full text-center pb-2 pt-2 shrink-0 z-20">
                <p
                    class="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
