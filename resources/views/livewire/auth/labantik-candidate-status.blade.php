<div x-data="{ 
    ...
    class="w-full max-w-2xl mx-auto my-auto px-4 relative">
    ...
    <div x-show="showVolumeModal" x-cloak
        class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/90">
        <div class="bg-gray-900 border border-gray-800 rounded-3xl p-8 max-w-md w-full shadow-2xl text-center space-y-6">
            <div class="w-20 h-20 bg-gray-800 text-primary-yellow rounded-full mx-auto flex items-center justify-center border border-gray-700">
    ...
    <!-- MAIN RESULT CARD -->
    <div class="bg-gray-900 border border-gray-800 rounded-3xl p-8 sm:p-10 shadow-2xl space-y-8">
        
        <!-- Header Info -->
        <div class="flex items-center justify-between border-b border-gray-800 pb-6">
            <div>
                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Peserta Seleksi</span>
                <h2 class="text-xl font-bold text-white uppercase">{{ $candidate->full_name }}</h2>
                <p class="text-xs font-semibold text-gray-400 uppercase">{{ $candidate->class_name }}</p>
            </div>
            <button wire:click="logoutCandidate" class="px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-xl text-xs font-bold transition-all">
                Keluar
            </button>
        </div>
        ...
        @if ($candidate->isFinalPassed())
            ...
            <div class="text-center space-y-6">
                <div class="w-20 h-20 bg-emerald-950/30 text-emerald-400 rounded-full mx-auto flex items-center justify-center border border-emerald-900/50">
                ...
                <div class="space-y-3">
                    <span class="px-4 py-1.5 bg-emerald-950/40 text-emerald-400 font-bold rounded-full text-xs uppercase tracking-widest border border-emerald-900/50">
                        Pengumuman Kelulusan
                    </span>
                    <h1 class="text-2xl sm:text-3xl font-black text-white uppercase italic tracking-tight">
                ...
                @if ($waGroupLink)
                    <div class="pt-4 border-t border-gray-800">
                        <a href="{{ $waGroupLink }}" target="_blank" rel="noopener noreferrer"
                            class="inline-flex items-center justify-center gap-3 w-full py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs sm:text-sm uppercase tracking-wider rounded-xl shadow-lg transition-all active:scale-95">
        ...
        @elseif ($candidate->final_status === 'pending')
            <!-- STATUS: PENDING -->
            <div class="text-center space-y-6">
                <div class="w-20 h-20 bg-amber-950/30 text-amber-400 rounded-full mx-auto flex items-center justify-center border border-amber-900/50">
                    <svg class="w-10 h-10 animate-spin" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    </svg>
                </div>
                <div class="space-y-3">
                    <span class="px-4 py-1.5 bg-amber-950/40 text-amber-400 font-bold rounded-full text-xs uppercase tracking-widest border border-amber-900/50">
                        Status Seleksi
                    </span>
        ...
        @else
            <!-- STATUS: TIDAK LOLOS -->
            <div class="text-center space-y-6">
                <div class="w-20 h-20 bg-gray-800 text-gray-400 rounded-full mx-auto flex items-center justify-center border border-gray-700">
                    <svg class="w-10 h-10" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                ...
                <div class="space-y-3">
                    <span class="px-4 py-1.5 bg-gray-800 text-gray-400 font-bold rounded-full text-xs uppercase tracking-widest border border-gray-700">
                        Pengumuman Seleksi
                    </span>
                    <h1 class="text-2xl sm:text-3xl font-black text-white uppercase italic tracking-tight">
