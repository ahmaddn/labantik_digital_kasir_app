<div class="w-full max-w-2xl mx-auto my-6 px-2 sm:px-4">
    ...
    <div class="bg-gray-900 rounded-[2.5rem] sm:rounded-[3.5rem] shadow-2xl border border-gray-800 overflow-hidden p-6 sm:p-10 md:p-14">
        
        <!-- Header Section -->
        <div class="text-center mb-10">
            <div class="inline-block mb-5">
                <div class="w-24 h-24 sm:w-28 sm:h-28 bg-gray-800 rounded-full p-4 flex items-center justify-center border border-gray-700 shadow-md">
                    <img src="{{ asset('labantik.png') }}" alt="Logo Labantik" class="w-16 h-16 sm:w-20 sm:h-20 object-contain">
                </div>
            </div>
        ...
        @if ($isSubmitted)
            ...
            <div class="text-center py-10 space-y-8">
                <div class="inline-block">
                    <div class="w-24 h-24 bg-emerald-950/20 text-emerald-400 rounded-full flex items-center justify-center mx-auto shadow-md border border-emerald-900/30">
                ...
                <div class="space-y-3">
                    ...
                    <p class="text-xs sm:text-sm text-gray-400 max-w-md mx-auto leading-relaxed font-semibold">
                        Terima kasih telah mendaftar. Data pendaftaran Anda telah tersimpan dengan aman di database sistem kami.
                    </p>
                </div>

                @if ($waGroupLink)
                    <!-- WhatsApp Link Box (Dark Mode Only) -->
                    <div class="p-6 sm:p-8 bg-emerald-950/20 border border-emerald-900 rounded-[2rem] max-w-md mx-auto shadow-md space-y-5">
                        <p class="text-xs sm:text-sm font-bold text-emerald-300 leading-relaxed">
                            Langkah Selanjutnya: Silakan gabung ke grup WhatsApp koordinasi calon anggota melalui tombol di bawah ini:
                        </p>
        ...
        @else
            ...
            <form wire:submit.prevent="submitRegistration" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    @error('fullName') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" wire:model="className" placeholder="Contoh: X RPL 2..."
                        class="w-full px-5 py-4 bg-gray-950 border border-gray-800 rounded-2xl focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow text-white text-xs sm:text-sm font-semibold transition-all">
                    @error('className') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model="phoneNumber" placeholder="Contoh: 081234567890..."
                            class="w-full px-5 py-4 bg-gray-950 border border-gray-800 rounded-2xl focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow text-white text-xs sm:text-sm font-semibold transition-all">
                        @error('phoneNumber') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" wire:model="parentPhoneNumber" placeholder="Contoh: 089876543210..."
                            class="w-full px-5 py-4 bg-gray-950 border border-gray-800 rounded-2xl focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow text-white text-xs sm:text-sm font-semibold transition-all">
                        @error('parentPhoneNumber') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" wire:model="illnessHistory" placeholder="Contoh: Asma, Alergi (kosongkan jika tidak ada)..."
                        class="w-full px-5 py-4 bg-gray-950 border border-gray-800 rounded-2xl focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow text-white text-xs sm:text-sm font-semibold transition-all">
                    @error('illnessHistory') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    @error('address') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea wire:model="reason" rows="4" placeholder="Tuliskan alasan, minat, dan motivasi utama Anda mendaftar masuk Labantik..."
                        class="w-full px-5 py-4 bg-gray-950 border border-gray-800 rounded-2xl focus:ring-2 focus:ring-primary-yellow focus:border-primary-yellow text-white text-xs sm:text-sm font-semibold transition-all leading-relaxed"></textarea>
                    @error('reason') <span class="text-xs text-red-500 font-bold mt-1 block ml-1">{{ $message }}</span> @enderror
                </div>
