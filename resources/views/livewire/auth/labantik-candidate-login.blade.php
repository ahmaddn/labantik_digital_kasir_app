<div class="w-full max-w-md mx-auto my-auto px-4">
    <!-- Full-screen dark background override -->
    <div class="fixed inset-0 bg-gray-950 -z-10"></div>

    <div class="bg-gray-900 rounded-3xl border border-gray-800 p-8 shadow-2xl space-y-6">
        <!-- Header -->
        <div class="text-center space-y-3">
            <div class="w-20 h-20 bg-gray-800 rounded-full p-3 flex items-center justify-center border border-gray-700 mx-auto">
                <img src="{{ asset('labantik.png') }}" alt="Logo Labantik" class="w-14 h-14 object-contain">
            </div>
            <div>
                <h2 class="text-2xl font-bold uppercase tracking-tight text-primary-yellow">
        ...
        @if ($errorMessage)
            <div class="p-4 bg-red-950/40 border border-red-800 rounded-2xl text-red-400 text-xs font-bold flex items-start gap-3">
        ...
        <form wire:submit.prevent="loginCandidate" class="space-y-4">
            <div>
                ...
                <input wire:model="firstName" type="text" required placeholder="Contoh: Ahmad"
                    class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl font-bold text-sm text-white focus:outline-none focus:ring-2 focus:ring-primary-blue transition-all">
            </div>
