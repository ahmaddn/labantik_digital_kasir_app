    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-2xl shadow-blue-900/5 border border-gray-100 dark:border-gray-700/50 p-6 md:p-8">
        ...
        <div class="mb-6">
            <div class="relative max-w-md">
                ...
                <input wire:model.live="search" type="text" placeholder="Cari nama atau label role..." class="w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary-blue dark:text-white transition-all text-sm">
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse min-w-[700px]">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-700">
                        <th class="pb-4 text-xs font-black uppercase tracking-widest text-gray-400 pl-4">ID Role (UUID)</th>
                        <th class="pb-4 text-xs font-black uppercase tracking-widest text-gray-400">Key Name (System)</th>
                        <th class="pb-4 text-xs font-black uppercase tracking-widest text-gray-400">Nama Tampilan (Label)</th>
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">
                    @forelse($roles as $role)
                        <tr class="group hover:bg-gray-50/50 dark:hover:bg-gray-900/30 transition-colors">
                            ...
                            <td class="py-5 text-sm font-mono text-gray-600 dark:text-gray-300">
                                {{ $role->name }}
                            </td>
                            <td class="py-5">
                                <span class="px-3 py-1 text-xs font-black rounded-full uppercase tracking-wider bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400">
                                    {{ $role->label }}
                                </span>
                            </td>
                            <td class="py-5 text-right pr-4">
                                <div class="flex items-center justify-end gap-2">
                                    <button wire:click="openEditModal('{{ $role->id }}')" class="p-2 text-gray-400 hover:text-primary-blue dark:hover:text-primary-yellow hover:bg-gray-100 dark:hover:bg-gray-900 rounded-xl transition-all">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    <button wire:click="confirmDelete('{{ $role->id }}')" class="p-2 text-gray-400 hover:text-primary-red hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-all" @if(in_array($role->name, ['superadmin', 'pengelola_jurusan', 'kasir'])) disabled style="opacity: 0.3;" @endif>
            ...
            @forelse($roles as $role)
                <div class="p-5 bg-gray-50 dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 space-y-3">
                    <div class="flex justify-between items-start">
                        <div>
                            <h3 class="font-bold text-gray-800 dark:text-white">{{ $role->label }}</h3>
                            <p class="text-xs font-mono text-gray-500 mt-1">Key: {{ $role->name }}</p>
                            <p class="text-[10px] text-gray-400 mt-1 break-all">ID: {{ $role->id }}</p>
                        </div>
                        <div class="flex items-center gap-1">
                            <button wire:click="openEditModal('{{ $role->id }}')" class="p-2 text-gray-400 hover:text-primary-blue dark:hover:text-primary-yellow hover:bg-white dark:hover:bg-gray-800 rounded-xl transition-all">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </button>
                            <button wire:click="confirmDelete('{{ $role->id }}')" class="p-2 text-gray-400 hover:text-primary-red hover:bg-white dark:hover:bg-gray-800 rounded-xl transition-all" @if(in_array($role->name, ['superadmin', 'pengelola_jurusan', 'kasir'])) disabled style="opacity: 0.3;" @endif>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </div>
                    </div>
    ...
    <div x-data="{ show: @entangle('showModal') }" x-show="show" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl sm:p-8 p-6 max-h-[90vh] sm:max-h-none overflow-y-auto sm:overflow-visible no-scrollbar border border-gray-100 dark:border-gray-700 z-10">
            ...
            <form wire:submit.prevent="saveRole" class="space-y-6">
                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2 ml-1">Key Name Sistem (e.g. bendahara)</label>
                    <input wire:model="name" type="text" required placeholder="Contoh: bendahara" class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary-blue dark:text-white transition-all text-sm">
                    @error('name') <span class="text-xs text-primary-red font-bold mt-1 block">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2 ml-1">Nama Tampilan Label (e.g. Bendahara TEFA)</label>
                    <input wire:model="label" type="text" required placeholder="Contoh: Bendahara TEFA" class="w-full px-4 py-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary-blue dark:text-white transition-all text-sm">
                    @error('label') <span class="text-xs text-primary-red font-bold mt-1 block">{{ $message }}</span> @enderror
                </div>
    ...
    <div x-data="{ showDelete: @entangle('showDeleteModal') }" x-show="showDelete" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div x-show="showDelete" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl sm:p-8 p-6 max-h-[90vh] sm:max-h-none overflow-y-auto sm:overflow-visible no-scrollbar border border-gray-100 dark:border-gray-700 z-10">
            <div class="text-center mb-6">
                ...
                <h2 class="text-2xl font-black text-gray-850 dark:text-white uppercase italic tracking-tight">Hapus Role</h2>
                <p class="text-gray-400 text-sm font-medium mt-2">Apakah Anda yakin ingin menghapus role ini secara permanen? Data hak akses user yang berkaitan dengan role ini juga akan terhapus.</p>
            </div>
            ...
            <div class="flex justify-center gap-3">
                ...
                <button wire:click="deleteRole" class="flex-1 py-4 bg-primary-red hover:bg-red-700 text-white rounded-2xl font-black text-sm uppercase italic tracking-wider transition-all shadow-xl shadow-red-500/10">
                    Ya, Hapus
                </button>
            </div>
        </div>
