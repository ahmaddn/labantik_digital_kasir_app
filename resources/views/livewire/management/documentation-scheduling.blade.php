        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-950/50 backdrop-blur-sm">
            <div class="bg-white dark:bg-gray-900 rounded-3xl p-6 max-w-lg w-full border border-gray-100 dark:border-gray-800 shadow-2xl space-y-6">
                ...
                <form wire:submit.prevent="randomizeSchedules" class="space-y-4">
                    ...
                    <!-- Non-Grade Cashiers Selection Box -->
                    @if(isset($noGradeCashiers) && $noGradeCashiers->count() > 0)
                        <div class="p-4 bg-amber-500/5 dark:bg-amber-950/20 rounded-2xl border border-amber-500/20 space-y-3">
                            <div class="flex items-center justify-between">
                                <label class="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    Pilih Anggota Tanpa Tingkat yang Ikut Randomize:
                                </label>
                                <span class="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">
                                    {{ count($selectedNoGradeUserIds) }} / {{ $noGradeCashiers->count() }} Terpilih
                                </span>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-44 overflow-y-auto p-1 bg-white/50 dark:bg-gray-900/50 rounded-xl border border-amber-500/10">
                                @foreach($noGradeCashiers as $ngc)
                                    <label class="flex items-center gap-2.5 p-2 rounded-lg hover:bg-amber-500/10 transition-colors cursor-pointer text-xs font-semibold text-gray-700 dark:text-gray-300">
                                        <input type="checkbox" value="{{ (string)$ngc->id }}" wire:model.live="selectedNoGradeUserIds" class="w-4 h-4 rounded text-amber-500 border-gray-300 focus:ring-amber-400">
                                        <span class="truncate">{{ $ngc->name }}</span>
                                    </label>
                                @endforeach
                            </div>
                            <p class="text-[10px] text-gray-400 italic">* Hanya anggota tanpa tingkat yang dicentang yang akan dimasukkan ke dalam acakan randomize.</p>
                        </div>
                    @endif
