<div class="p-6 max-w-4xl mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div
            ...
            <div class="space-y-6">
                <!-- Select Month -->
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Pilih Bulan
                        Yang Akan Ditutup</label>
                    <select wire:model.live="selectedMonth"
                        ...
                        @foreach ($availableMonths as $month)
                            <option value="{{ $month }}">
                                {{ \Carbon\Carbon::parse($month . '-01')->translatedFormat('F Y') }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                @if ($isClosed)
                    ...
                    <div
                        class="bg-emerald-500/10 border-2 border-dashed border-emerald-500 text-emerald-600 dark:text-emerald-400 p-6 rounded-2xl text-center">
                        ...
                        <p class="text-xs font-bold mt-1 opacity-80">Seluruh transaksi di bulan
                            {{ \Carbon\Carbon::parse($selectedMonth . '-01')->translatedFormat('F Y') }} telah berhasil
                            dikunci dan diarsipkan.</p>
                    </div>
                    ...
                    @elseif($cancelBlockedReason)
                        <div
                            class="bg-amber-500/10 border-2 border-dashed border-amber-500 text-amber-700 dark:text-amber-400 p-4 rounded-2xl text-center">
                ...
                @else
                    ...
                    <div
                        class="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-3xl space-y-6 border border-gray-100 dark:border-gray-800">
                        <h3 class="text-xs font-black uppercase tracking-widest text-slate-500">PENYESUAIAN SALDO BAWAAN
                            (CARRY FORWARD)</h3>
                        <p class="text-xs font-bold text-gray-400 leading-relaxed">
                            Masukkan nominal uang kas bawaan riil yang akan dipindahkan ke awal bulan berikutnya.
                            Nominal di bawah ini otomatis terisi dari saldo akhir kas sistem bulan ini, namun dapat
                            disesuaikan jika ada selisih uang fisik.
                        </p>

                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Saldo
                                    Awal Modal Baru (Rp)</label>
                    ...
                    <!-- Action Button -->
                    <button wire:click="closeMonth" @disabled(!$canClose)
        ...
        <div
            ...
            <div>
                ...
                <h2 class="text-lg font-black uppercase italic text-white">
                    {{ \Carbon\Carbon::parse($selectedMonth . '-01')->translatedFormat('F Y') }}</h2>
                ...
                <div class="space-y-4 mt-8">
                    <div class="flex justify-between items-center py-2 border-b border-dashed border-gray-800">
                        ...
                        <span
                            class="font-black text-sm text-gray-200">Rp{{ number_format($monthStats['modal'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-dashed border-gray-800">
                        <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">Kas Keuntungan</span>
                        <span
                            class="font-black text-sm text-emerald-400">Rp{{ number_format($monthStats['profit'] ?? 0, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-12 pt-6 border-t border-gray-800">
                ...
                <p class="text-3xl font-black italic text-white">
                    Rp{{ number_format($monthStats['total'] ?? 0, 0, ',', '.') }}</p>
            </div>

            @if ($isClosed)
                <div class="mt-6">
                    <a href="{{ route('transactions', ['showArchived' => 1, 'filterMonth' => $selectedMonth]) }}"
                        class="inline-flex items-center justify-center w-full py-4 px-6 bg-white text-primary-blue rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-500/20 hover:bg-slate-50 transition-all">
                        Lihat Arsip Bulan Ini
                    </a>
                </div>
            @endif
        </div>
    ...
    <div x-data="{ show: @entangle('showCancelConfirmation') }" x-show="show" x-cloak
        ...
        <div @click.away="show = false"
            ...
            <div class="p-8 bg-gray-50 dark:bg-gray-800/50 flex gap-4">
                ...
                <button wire:click="cancelClosing"
                    class="flex-1 py-4 bg-amber-500 text-white rounded-2xl font-black italic uppercase text-xs tracking-widest shadow-xl shadow-amber-500/20 hover:scale-105 active:scale-95 transition-all">
                    Ya, Batalkan
                </button>
            </div>
        </div>
