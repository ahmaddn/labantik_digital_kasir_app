    ...
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-4 space-y-1 mt-4 overflow-y-auto no-scrollbar pb-10">
            ...
            @if (session('active_role_name') !== 'superadmin')
                ...
                @if ($hasHigherRole)
                    @if ($isSessionFinished || $hasClockedOut)
                        <a href="{{ route('kasir') }}" class="flex items-center px-6 py-5 text-sm font-black rounded-[2rem] bg-amber-500 text-black shadow-2xl shadow-amber-500/30 hover:scale-105 active:scale-95 transition-all uppercase italic tracking-wider">
                            <flux:icon.clock class="w-6 h-6 mr-4 text-black shrink-0 animate-pulse" />
                            Kasir (Pasca-Closing)
                        </a>
                    @else
                    ...
                    @endif
                @elseif(!$isScheduled)
                    <div class="flex flex-col gap-2 px-6 py-4 rounded-[2rem] bg-amber-500/10 border border-amber-500/30 text-amber-600 dark:text-amber-400 shadow-xl opacity-90 cursor-not-allowed">
                        <div class="flex items-center text-xs font-black uppercase italic tracking-wider">
                            <svg class="w-5 h-5 mr-3 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
                            Tidak Ada Jadwal
                        </div>
                        <span class="text-[9px] font-bold leading-normal uppercase">Anda tidak dijadwalkan jaga kasir hari ini.</span>
                    </div>
                @elseif($hasClockedOut || $isSessionFinished)
                    {{-- Sesi sudah selesai (clock out atau daily recap sudah ditutup) --}}
                ...
                @else
                    {{-- Terjadwal, belum clock out, sesi belum selesai → bisa buka kasir --}}
                    <a href="{{ route('kasir') }}" class="flex items-center px-6 py-5 text-sm font-black rounded-[2rem] bg-primary-red text-white shadow-2xl shadow-red-500/30 hover:scale-105 active:scale-95 transition-all uppercase italic tracking-wider">
                        <svg class="w-6 h-6 mr-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
                        Buka Mode Kasir
                    </a>
            ...
            @if (session('active_role_name') === 'superadmin' ||
                ...
                <a href="{{ route('documentation-schedules') }}"
                    class="flex items-center px-4 py-2.5 text-xs font-semibold rounded-xl transition-all {{ request()->routeIs('documentation-schedules') ? 'bg-primary-blue text-white shadow-xl shadow-blue-900/20' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800' }}">
                    <svg class="w-5 h-5 mr-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
                        stroke-linecap="round" stroke-linejoin="round">
